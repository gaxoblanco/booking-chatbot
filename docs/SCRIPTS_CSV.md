...
docker exec -it whatsapp-demo python scripts/csv/load_professionals_from_csv.py /app/data/csv/profesionales_demo.csv

# Simular sin guardar
docker exec -it whatsapp-demo python scripts/csv/load_professionals_from_csv.py /app/data/csv/profesionales_demo.csv --dry-run

# Generar template CSV de ejemplo
docker exec -it whatsapp-demo python scripts/csv/load_professionals_from_csv.py --template


# ── PACIENTES ──────────────────────────────────────────────────────────────────

# Cargar (interactivo: pregunta CSV y duración)
docker exec -it whatsapp-demo python scripts/csv/load_patients_from_csv.py

# Cargar con path directo (solo pregunta duración)
docker exec -it whatsapp-demo python scripts/csv/load_patients_from_csv.py /app/data/csv/pacientes_demo.csv

# Cargar con todo fijo, sin preguntas
docker exec -it whatsapp-demo python scripts/csv/load_patients_from_csv.py /app/data/csv/pacientes_demo.csv --weeks 12

# Simular sin guardar
docker exec -it whatsapp-demo python scripts/csv/load_patients_from_csv.py /app/data/csv/pacientes_demo.csv --dry-run

# Generar template CSV de ejemplo
docker exec -it whatsapp-demo python scripts/csv/load_patients_from_csv.py --template


# ── LIMPIEZA ───────────────────────────────────────────────────────────────────

# Borrar pacientes (usa el mismo CSV que se usó para cargar)
docker exec -it whatsapp-demo python scripts/csv/delete_patients_from_csv.py /app/data/csv/pacientes_demo.csv

# Simular borrado sin eliminar nada
docker exec -it whatsapp-demo python scripts/csv/delete_patients_from_csv.py /app/data/csv/pacientes_demo.csv --dry-run


# ── EMAILS DE INVITACIÓN CALENDAR (standalone) ─────────────────────────────────

# Enviar a profesionales sin calendar_id configurado
docker exec -it whatsapp-demo python scripts/csv/send_calendar_invitations.py

# Enviar a todos
docker exec -it whatsapp-demo python scripts/csv/send_calendar_invitations.py --all
```
...
python scripts/csv/load_patients_from_csv.py
docker exec whatsapp-demo python scripts/csv/load_patients_from_csv.py   # sin -it falla el input()
```
...
Toma el mismo CSV usado para cargar y elimina todo: cancela los eventos en Google Calendar, elimina los calendarios secundarios `Turnos - X` de los profesionales involucrados, limpia el `calendar_id` en BD (quedan como pendientes) y borra clientes y citas de la BD. Si el evento o calendario ya no existe en Calendar, continúa sin romper.

### `validate_pending_calendars.py`

Re-valida el acceso a Google Calendar de profesionales que quedaron pendientes al cargar el CSV. Un profesional queda pendiente cuando no tenía su calendario compartido al momento de la carga. Correrlo manualmente cuando el profesional confirma que ya compartió su calendario.

**Profesional pendiente:** tiene `calendar_email` en BD pero `calendar_id` es NULL.

**Al resolver:** crea el calendario secundario `Turnos - <Nombre>`, lo comparte con el profesional y guarda el `calendar_id` en BD.
...
| Error | Causa | Fix |
|-------|-------|-----|
| `sqlite3.OperationalError: no such column` | Script corrido fuera del Docker | Usar `docker exec -it whatsapp-demo python scripts/csv/...` |
...
| `Name or service not known` en email | SMTP no configurado o variables de entorno no cargadas | Verificar `.env` y reiniciar el contenedor |